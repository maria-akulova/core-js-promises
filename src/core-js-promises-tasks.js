...
function getFirstPromiseResult(promises) {
  return Promise.race(promises).then((winner) => winner.valueOf());
}
...
function getAllOrNothing(promises) {
  const result = Promise.all(promises).then((responses) => {
    responses.filter((response) => response.status === 'rejected');
  });
  return result.length < promises.length ? result[0] : result;
}
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((responses) => {
    return responses.map((response) =>
      response.status === 'rejected' ? null : response.value
    );
  });
}
...
async function queuPromises(promises) {
  return promises.reduce(async (prevPromise, currentPromise) => {
